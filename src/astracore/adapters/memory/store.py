...
class SQLMemoryStore(MemoryStore):
    ...
    async def bind_conversation(self, binding: ConversationProjectBinding) -> ConversationProjectBinding:
        now = datetime.now(UTC)
        async with get_session(self._db_url) as db:
    ...
    async def get_conversation_binding(self, conversation_id: UUID) -> ConversationProjectBinding | None:
        async with get_session(self._db_url) as db:
