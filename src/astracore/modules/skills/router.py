...
class SkillRouter:
    ...
    async def _route_vector(self, message: str) -> list[SkillRow]:
        ...
        try:
            ...
            scores: list[tuple[float, SkillRow]] = [
                (float(np.dot(msg_vec, vec)), row) for row, vec in self._vectors.values()
            ]
            ...
            for i, (score, row) in enumerate(scores):
                if i == 0:
                    ...
                    if result and score >= cfg.secondary_threshold and len(result) < cfg.max_skills:
                        result.append(row)
                    else:
                        break
            return result
    ...
    def _get_llm_adapter(self) -> Any:
        if self._llm_adapter is None:
            from astracore.infrastructure.llm.anthropic import AnthropicAdapter  # noqa: PLC0415
            from astracore.infrastructure.llm.openai import OpenAIAdapter  # noqa: PLC0415
    ...
    async def _route_llm(self, message: str) -> list[SkillRow]:
        ...
        if not skills:
            return []
        try:
            from astracore.modules.chat.domain.message import Message, MessageRole  # noqa: PLC0415

            skill_lines = "\n".join(
                f'- id: "{s.id}", name: "{s.name}", description: "{s.description}"' for s in skills
            )
