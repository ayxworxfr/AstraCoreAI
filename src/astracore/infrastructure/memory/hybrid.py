...
class HybridMemoryAdapter(MemoryAdapter):
    ...
    async def _save_short_term_to_db(
        ...
        from astracore.infrastructure.db.models import ChatSessionRow
    ...
    async def _load_short_term_from_db(self, session_id: UUID) -> list[dict[str, Any]] | None:
    ...
    async def search_memory(
        ...
        try:
            ...
            engine = self._get_db()
            async with AsyncSession(engine) as db:
                stmt = select(MemoryEntryRow).where(MemoryEntryRow.content.ilike(f"%{query}%"))
